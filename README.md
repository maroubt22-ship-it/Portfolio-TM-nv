...
- **Contact form behavior:** the form submits asynchronously as JSON to FormSubmit and forwards messages to `taoufiq.maroub25@gmail.com` without redirecting visitors to Gmail. It displays an in-page confirmation only when FormSubmit returns an explicit success response; otherwise it displays an activation error. FormSubmit sends a one-time activation email the first time the form is used; confirm it before accepting public messages, including checking Gmail Spam if needed.
...
4. Copy the `/exec` deployment URL into the `data-endpoint` attribute on the `contactForm` in `index.html`.
5. Push the change and test the form from the hosted site.

The Apps Script sends the submitted name, email, subject, and message with `GmailApp.sendEmail`, and the form displays success only after the endpoint confirms that Gmail accepted the message. The endpoint URL cannot be generated from this repository because Google requires authorization in your account.
